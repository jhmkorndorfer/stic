...
void Profile_ctx(AtomicLine *line, RHContext *ctx)
{
  ...
  const char routineName[] = "Profile_ctx";
  register int la, k, mu, n, to_obs, nz;
  ...
  if (line->PRD){//&& line->rho_prd == NULL) {
    if(line->rho_prd == NULL){
      if (inputLocal->PRD_angle_dep)
	Nlamu = 2*atmosLocal->Nrays * line->Nlambda;
      else
	Nlamu = line->Nlambda;
      if(!line->rho_prd) freeMatrix((void**)line->rho_prd);
	line->rho_prd = matrix_double(Nlamu, atmosLocal->Nspace);
      //else memset(line->rho_prd,0,Nlamu*atmosLocal->Nspace*sizeof(double));
    ...
    if(!line->Qelast) line->Qelast = (double *) calloc(atmosLocal->Nspace, sizeof(double));
    else memset(line->Qelast,0,atmosLocal->Nspace*sizeof(double));    
  ...
  vbroad = atom->vbroad;
  adamp  = (double *) calloc(atmosLocal->Nspace, sizeof(double));
  if (line->Voigt) Damping_ctx(line, adamp, ctx);

  if(!line->wphi) line->wphi = (double *) calloc(atmosLocal->Nspace, sizeof(double));
  else memset(line->wphi, 0,atmosLocal->Nspace*sizeof(double));
  ...
  if (inputLocal->limit_memory) {
    ...
    if (line->polarizable && (inputLocal->StokesMode > FIELD_FREE)) {
      NrecStokes = (inputLocal->magneto_optical) ? 7 : 4;
      phi = (double *) calloc(NrecStokes*atmosLocal->Nspace, sizeof(double));
	...
	psi_V = phi + 6*atmosLocal->Nspace;
      ...
      NrecStokes = 1;
      phi = (double *) calloc(atmosLocal->Nspace, sizeof(double));
    }
  } else {
    if (atmosLocal->moving || 
	(line->polarizable && (inputLocal->StokesMode > FIELD_FREE))) {
      Nlamu = 2*atmosLocal->Nrays*line->Nlambda;
      
      if(!line->phi) line->phi = matrix_double(Nlamu, atmosLocal->Nspace);
      else memset(line->phi[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));

      if (line->polarizable && (inputLocal->StokesMode > FIELD_FREE)) {
	if(!line->phi_Q) line->phi_Q = matrix_double(Nlamu, atmosLocal->Nspace);
	else memset(line->phi_Q[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));

	if(!line->phi_U) line->phi_U = matrix_double(Nlamu, atmosLocal->Nspace);
	else memset(line->phi_U[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));
	
	if(!line->phi_V) line->phi_V = matrix_double(Nlamu, atmosLocal->Nspace);
	else memset(line->phi_V[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));

	if (inputLocal->magneto_optical) {
	  if(!line->psi_Q) line->psi_Q = matrix_double(Nlamu, atmosLocal->Nspace);
	  else memset(line->psi_Q[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));

	  if(!line->psi_U) line->psi_U = matrix_double(Nlamu, atmosLocal->Nspace);
	  else memset(line->psi_U[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));
	  
	  if(!line->psi_V) line->psi_V = matrix_double(Nlamu, atmosLocal->Nspace);
	  else memset(line->psi_V[0], 0, Nlamu*atmosLocal->Nspace*sizeof(double));

	}
      }
    } else
       if(!line->phi) line->phi = matrix_double(line->Nlambda, atmosLocal->Nspace);
       else memset(line->phi[0], 0, line->Nlambda*atmosLocal->Nspace*sizeof(double));
  ...
  if (line->polarizable && (inputLocal->StokesMode > FIELD_FREE)) {
    ...
    vB = (double *) calloc(atmosLocal->Nspace, sizeof(double));
    sv = (double *) calloc(atmosLocal->Nspace, sizeof(double));

    for (k = 0;  k < atmosLocal->Nspace;  k++) {
      vB[k] = Larmor * atmosLocal->B[k] / vbroad[k];
      sv[k] = 1.0 / (SQRTPI * vbroad[k]);
    }
  }
  ...
  if (atmosLocal->moving ||
    ...
    v_los = matrix_double(atmosLocal->Nrays, atmosLocal->Nspace);
    for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
      for (k = 0;  k < atmosLocal->Nspace;  k++) {
	v_los[mu][k] = vproject_ctx(k, mu, ctx) / vbroad[k];
      }
    }
    v = matrix_double(atmosLocal->Nspace, line->Ncomponent);
	...
	for (k = 0;  k < atmosLocal->Nspace;  k++) {
      ...
      for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
	wlamu = getwlambda_line(line, la) * 0.5*atmosLocal->wmu[mu];
	...
	for (to_obs = 0;  to_obs <= 1;  to_obs++) {
	  ...
	  if (inputLocal->limit_memory) {
	    for (k = 0;  k< NrecStokes*atmosLocal->Nspace;  k++) phi[k] = 0.0;
	  } else {
	    phi = line->phi[lamu];
	    if (line->polarizable && (inputLocal->StokesMode > FIELD_FREE)) {
	      phi_Q = line->phi_Q[lamu];
	  ...
	  if (line->polarizable && (inputLocal->StokesMode > FIELD_FREE)) {
		...
		phi[k]   += (phi_delta*sin2_gamma + 0.5*phi_sigma) * sv[k];
		phi_Q[k] += sign *
		  phi_delta * sin2_gamma * atmosLocal->cos_2chi[mu][k] * sv[k];
		phi_U[k] +=
		  phi_delta * sin2_gamma * atmosLocal->sin_2chi[mu][k] * sv[k];
		phi_V[k] += sign *  line->c_fraction[n] * 
		  0.5*(phi_sp - phi_sm) * atmosLocal->cos_gamma[mu][k] * sv[k];
		...
		if (inputLocal->magneto_optical) {
		  ...
		  psi_Q[k] += sign *
		    psi_delta * sin2_gamma * atmosLocal->cos_2chi[mu][k] * sv[k];
		  psi_U[k] +=
		    psi_delta * sin2_gamma * atmosLocal->sin_2chi[mu][k] * sv[k];
		  psi_V[k] += sign * line->c_fraction[n] * 
		    0.5 * (psi_sp - psi_sm) * atmosLocal->cos_gamma[mu][k] * sv[k];
		}
	      }
              /* --- Ensure proper normalization of the profile -- -- */
	    ...
	    if(inputLocal->fast_isotopic_split){
	      /* --- Field-free case --                  -------------- */
	      for (k = 0;  k < atmosLocal->Nspace;  k++) {
		dum = (line->lambda[la] - line->lambda0 ) * CLIGHT / (vbroad[k] * line->lambda0);
		vk = dum + sign * v_los[mu][k];	
	  ...
	  if (inputLocal->limit_memory) writeProfile_ctx(line, lamu, phi, ctx);
	}
      }
    }
  } else {
    ...
    for (la = 0;  la < line->Nlambda;  la++) {
      wlamu = getwlambda_line(line, la);
      if (inputLocal->limit_memory)
	for (k = 0;  k < atmosLocal->Nspace;  k++) phi[k] = 0.0;
      else
	phi = line->phi[la];
	...
	line->wphi[k] += phi[k] * wlamu;
      ...
      if (inputLocal->limit_memory) writeProfile_ctx(line, la, phi, ctx);
    }
  }
  /* --- Store the inverse of the profile normalization -- ---------- */

  for (k = 0;  k < atmosLocal->Nspace;  k++) line->wphi[k] = 1.0 / line->wphi[k];

  /* --- Clean up --                                     ------------ */

  free(adamp);
  if (inputLocal->limit_memory) free(phi);
...
void MolecularProfile_ctx(MolecularLine *mrt, RHContext *ctx)
{
  ...
  if(!mrt->wphi) mrt->wphi = (double *) calloc(atmosLocal->Nspace, sizeof(double));
  else           memset((double*)mrt->wphi,0,atmosLocal->Nspace*sizeof(double));
    
  adamp = (double *) calloc(atmosLocal->Nspace, sizeof(double));
  if (mrt->Voigt) MolecularDamping_ctx(mrt, adamp, ctx);

  if (atmosLocal->moving) {
    v = (double *) malloc(atmosLocal->Nspace * sizeof(double));
    ...
    for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
      for (k = 0;  k < atmosLocal->Nspace;  k++)
	v_los[mu][k] = vproject_ctx(k, mu, ctx) / molecule->vbroad[k];
    }
    /* --- Initialize permanent storage for line profiles -- -------- */

    if(!mrt->phi) mrt->phi = matrix_double(2*atmosLocal->Nrays * mrt->Nlambda, atmosLocal->Nspace);
    else          memset(mrt->phi[0], 0, 2*atmosLocal->Nrays * mrt->Nlambda* atmosLocal->Nspace*sizeof(double));
    
  } else{
    if(!mrt->phi)  mrt->phi = matrix_double(mrt->Nlambda, atmosLocal->Nspace);
    else           memset(mrt->phi, 0, mrt->Nlambda*atmosLocal->Nspace*sizeof(double));
  }
  /* --- Temporary storage for Doppler width scaling -- ------------- */  

  sv = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  for (k = 0;  k < atmosLocal->Nspace;  k++)
    sv[k] = 1.0 / (SQRTPI * molecule->vbroad[k]);
  ...
  if (atmosLocal->moving) {
	...
	v[k] = (mrt->lambda[la] - mrt->lambda0) * CLIGHT /
      ...
      for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
	wlamu = getwlambda_mrt(mrt, la) * 0.5*atmosLocal->wmu[mu];

	/* --- First the downward, then the upward direction -- ----- */

	phi_down = mrt->phi[2*(atmosLocal->Nrays*la + mu)];
	phi_up   = mrt->phi[2*(atmosLocal->Nrays*la + mu) + 1];
  ...
  for (k = 0;  k < atmosLocal->Nspace;  k++) mrt->wphi[k] = 1.0 / mrt->wphi[k];
