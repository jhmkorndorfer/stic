...
void CollisionRate_ctx(struct Atom *atom, char **fp_atom, RHContext *ctx)
{
  ...
  int     nitem, i1, i2, i, j, ij, ji, Nlevel = atom->Nlevel, Nitem,
          status;
  long    Nspace = atmosLocal->Nspace, offset = 0;
  //fpos_t  collpos;
  double  dE, C0, *T, *coeff, *C=NULL, Cdown, Cup, gij, *np, xj, fac, fxj;
  ...
  while ((status = getLine2(fp_atom[offset++], COMMENT_CHAR,
	...
	!strcmp(keyword, "CH") ) {
      ...
      if (Nitem > 3) {
	//splineCoef(Nitem, T, coeff);
	//splineEval(Nspace, atmosLocal->T, C, hunt=TRUE);
	splineHermite(Nitem, T, coeff, Nspace, atmosLocal->T, C);
      } else
	Linear(Nitem, T, coeff, Nspace, atmosLocal->T, C, hunt=TRUE);
    }
    ...
    if (!strcmp(keyword, "OMEGA")) {
      ...
      for (k = 0;  k < Nspace;  k++) {
        Cdown = C0 * atmosLocal->ne[k] * C[k] /
                                 (atom->g[j] * sqrt(atmosLocal->T[k]));
	atom->C[ij][k] += Cdown;
	atom->C[ji][k] += Cdown * atom->nstar[j][k]/atom->nstar[i][k];
      }
    } else if (!strcmp(keyword, "CE")) {      
      ...
      gij = atom->g[i] / atom->g[j];
      for (k = 0;  k < Nspace;  k++) {
        Cdown = C[k] * atmosLocal->ne[k] * gij * sqrt(atmosLocal->T[k]);
	atom->C[ij][k] += Cdown;
	atom->C[ji][k] += Cdown * atom->nstar[j][k]/atom->nstar[i][k];
      }
    } else if (!strcmp(keyword, "CI")) {      
      ...
      dE = atom->E[j] - atom->E[i];
      for (k = 0;  k < Nspace;  k++) {
        Cup = C[k] * atmosLocal->ne[k] *
	  exp(-dE/(KBOLTZMANN*atmosLocal->T[k])) * sqrt(atmosLocal->T[k]);
	atom->C[ji][k] += Cup;
	atom->C[ij][k] += Cup * atom->nstar[i][k]/atom->nstar[j][k];
      }
    } else if (!strcmp(keyword, "CP")) {
	...
	atom->C[ij][k] += Cup * atom->nstar[i][k]/atom->nstar[j][k];
      ...
      for (k = 0;  k < Nspace;  k++)
	atom->C[ij][k] += atmosLocal->H->n[0][k] * C[k];
	...
	summrs = sumscl * summers(i, j, atmosLocal->ne[k], atom) +
	  (1.0 - sumscl);
	tg = atmosLocal->T[k];
	...
	cdn *= atmosLocal->ne[k] * CUBE(CM_TO_M);
	cup *= atmosLocal->ne[k] * CUBE(CM_TO_M);
	...
	atom->C[ji][k] += cup;
      ...
      for (k = 0;  k < Nspace;  k++) {
	summrs = sumscl*summers(i, j, atmosLocal->ne[k], atom) + (1.0-sumscl);
	tg = atmosLocal->T[k];
	...
	cdn *= atmosLocal->ne[k] * summrs * CUBE(CM_TO_M);
	cup  = cdn * atom->nstar[j][k]/atom->nstar[i][k];
	...
	atom->C[ji][k] += cup;
      ...
      for (k = 0;  k < Nspace;  k++) {
	if (atmosLocal->T[k] >= ar85t1  &&  atmosLocal->T[k] <= ar85t2) {

	  t4 = atmosLocal->T[k] / 1.0E4;
	  cup = ar85a * 1e-9 * pow(t4,ar85b) * exp(-ar85c*t4) *
	    exp(-ar85d*EV/KBOLTZMANN/atmosLocal->T[k])*atmosLocal->H->n[atmosLocal->H->Nlevel-1][k] *
	    CUBE(CM_TO_M);
	  atom->C[ji][k] += cup;
	}
      }
	...
	if (atmosLocal->T[k] >= ar85t1  &&  atmosLocal->T[k] <= ar85t2) {
	  ...
	  t4 = atmosLocal->T[k] / 1.0E4;
	  cdn = ar85a * 1E-9 * pow(t4, ar85b) * (1.0 + ar85c*exp(ar85d * t4)) * 
	    atmosLocal->H->n[0][k] * CUBE(CM_TO_M);
	  atom->C[ij][k] += cdn;
	}
      }
    } else if (!strcmp(keyword, "BURGESS")) {
      ...
      for (k = 0;  k < Nspace;  k++) {
	dekt = de * EV / (KBOLTZMANN * atmosLocal->T[k]);
	dekt = MIN(500, dekt);
	dekti = 1.0 / dekt;
        wlog = log(1.0 + dekti);
	wb = pow(wlog, betab / (1.0 + dekti));
	cup = 2.1715E-8 * cbar * pow(13.6/de, 1.5) * sqrt(dekt) *
          E1(dekt) * wb * atmosLocal->ne[k] * CUBE(CM_TO_M);
...
{
  const char routineName[] = "CollisionRateOne_ctx";
  register int  n, m, ii;
  ...
  while ((status = getLine2(fp_atom[offset++], COMMENT_CHAR,
	...
	!strcmp(keyword, "CH") ) {
      ...
      if (Nitem > 3) {
	//splineCoef(Nitem, T, coeff);
	//splineEval(1, &atmosLocal->T[k], &C, hunt=TRUE);
	splineHermite(Nitem, T, coeff, 1, &atmosLocal->T[k], &C); //POSSIBLE BUGS HERE!! race condition
      } else
	Linear(Nitem, T, coeff, 1, &atmosLocal->T[k], &C, hunt=TRUE);
    }
	...
	(atom->g[j] * sqrt(atmosLocal->T[k]));
      ...
      gij = atom->g[i] / atom->g[j];
      //for (k = 0;  k < Nspace;  k++) {
      Cdown = C * atmosLocal->ne[k] * gij * sqrt(atmosLocal->T[k]);
      atom->C[ij][k] += Cdown;
      atom->C[ji][k] += Cdown * atom->nstar[j][k]/atom->nstar[i][k];
      //}
    } else if (!strcmp(keyword, "CI")) {      
	...
	//  }
      ...
      //   for (k = 0;  k < Nspace;  k++)
      atom->C[ij][k] += atmosLocal->H->n[0][k] * C;
	...
	summrs = sumscl * summers_ctx(i, j, atmosLocal->ne[k], atom, ctx) +
	  (1.0 - sumscl);
	tg = atmosLocal->T[k];
	...
	// }
      ...
      //   for (k = 0;  k < Nspace;  k++) {
	summrs = sumscl*summers_ctx(i, j, atmosLocal->ne[k], atom, ctx) + (1.0-sumscl);
	tg = atmosLocal->T[k];
