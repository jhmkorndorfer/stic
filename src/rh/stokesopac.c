...
void StokesK_ctx(int nspect, int k, double chi_I, double K[4][4], RHContext *ctx)
{
  ...
  if (containsPolarized(as)) {
    K[0][1] = as->chi[atmosLocal->Nspace + k];
    K[0][2] = as->chi[2*atmosLocal->Nspace + k];
    K[0][3] = as->chi[3*atmosLocal->Nspace + k];

    if (inputLocal->magneto_optical) {
      K[1][2] = as->chip[2*atmosLocal->Nspace + k];
      K[1][3] = as->chip[atmosLocal->Nspace + k];
      K[2][3] = as->chip[k];
    }
  }
  if (atmosLocal->backgrflags[nspect].ispolarized) {
    K[0][1] += as->chi_c[atmosLocal->Nspace + k];
    K[0][2] += as->chi_c[2*atmosLocal->Nspace + k];
    K[0][3] += as->chi_c[3*atmosLocal->Nspace + k];

    if (inputLocal->magneto_optical) {
      K[1][2] += as->chip_c[2*atmosLocal->Nspace + k];
      K[1][3] += as->chip_c[atmosLocal->Nspace + k];
      K[2][3] += as->chip_c[k];
    }
  }
  /* --- Divide by Stokes I opacity and fill lower diagonal part -- - */
