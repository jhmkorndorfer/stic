...
void zeroRadiation_ctx(Atom *atom, int nact, RHContext *ctx)
{
  ...
  initGammaAtom_ctx(atom, 1.0, ctx);
  ...
  statEquil_ctx(atom, (inputLocal->isum == -1) ? 0 : inputLocal->isum, ctx);
}
...
// extern CommandLine commandline;
  ...
  const char routineName[] = "initSolution_alloc2_ctx";
  register int nspect, nact,k,kr,mu,la;
  char    permission[3], file_imu[MAX_MESSAGE_LENGTH];
  int     Nsr, Nplane, index, oflag;
  int to_obs,sign,ncoef,ilow,Nlamu,lamu,Np;
  ...
  if (!inputLocal->limit_memory) {
    if (spectrumLocal->J != NULL) freeMatrix((void **) spectrumLocal->J);
    spectrumLocal->J = matrix_double(spectrumLocal->Nspect, atmosLocal->Nspace);

    /* --- If we do background polarization we need space for the
           anisotropy --                               -------------- */

    if (inputLocal->backgr_pol){
      if (spectrumLocal->J20 != NULL) freeMatrix((void **) spectrumLocal->J20);
      spectrumLocal->J20 = matrix_double(spectrumLocal->Nspect, atmosLocal->Nspace);
    }
  }

 /* --- For the PRD angle approximation  we need to store J in
     the gas frame,                                   -------- */
  if (inputLocal->PRD_angle_dep == PRD_ANGLE_APPROX &&  atmosLocal->NPRDactive > 0) {
    
    //if (spectrumLocal->Jgas != NULL) freeMatrix((void **) spectrumLocal->Jgas);
    //spectrumLocal->Jgas  = matrix_double(spectrumLocal->Nspect, atmosLocal->Nspace);
    if (spectrumLocal->v_los != NULL) freeMatrix((void **) spectrumLocal->v_los);
    spectrumLocal->v_los = matrix_double(    atmosLocal->Nrays, atmosLocal->Nspace);     
    
    /* Calculate line of sight velocity */
    for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
      for (k = 0;  k < atmosLocal->Nspace;  k++) {
	spectrumLocal->v_los[mu][k] = vproject_ctx(k, mu, ctx); // / vbroad[k];
      }
    }
	...
	if (line->PRD) {
	  ...
	  //line->gII  = (double **) malloc(atmosLocal->Nrays * sizeof(double *));
	  for (k = 0;  k < atmosLocal->Nspace;  k++) {
	    for (la = 0 ;  la < line->Nlambda;  la++) {
	...
	*/
	  ...
	  Nsr=3*MAX(3*PRD_QWING,2*PRD_QSPREAD)/PRD_DQ+1;
	  if (line->gII != NULL) freeMatrix((void **) line->gII);
	  line->gII  = matrix_double(atmosLocal->Nspace*line->Nlambda,Nsr);
	  line->gII[0][0] = -1.0;
	}
      }
    }
   ...
   /* precompute prd_rho interpolation coefficients if requested */
	...
	for (kr = 0;  kr < atom->Nline;  kr++) {
	  ...
	  if (line->PRD) {
	    ...
	    Nlam = 2*atmosLocal->Nrays * line->Nlambda;

	    if (line->frac != NULL) freeMatrix((void **) line->frac);
	    line->frac = matrix_double(Nlam, atmosLocal->Nspace);

	    if (line->id0 != NULL) freeMatrix((void **) line->id0);
	    line->id0  = matrix_int(Nlam, atmosLocal->Nspace);

	    if (line->id1 != NULL) freeMatrix((void **) line->id1);
	    line->id1  = matrix_int(Nlam, atmosLocal->Nspace);

	    //if(line->Jgas != NULL) del_d2dim(line->Jgas, 0, 0);
	    //line->Jgas = d2dim(0, line->Nlambda-1, 0, atmosLocal->Nspace-1);
      ...
      Nlamu=2*2*atmosLocal->Nrays*(spectrumLocal->nJlam)*atmosLocal->Nspace;
      
      if(spectrumLocal->iprdh != NULL) free((void*)spectrumLocal->iprdh);
      spectrumLocal->iprdh =  ((unsigned int *)calloc( Nlamu , sizeof(unsigned int   )));
      
      if(spectrumLocal->cprdh != NULL) free((void*)spectrumLocal->cprdh);
      spectrumLocal->cprdh =  ((double *)calloc( Nlamu , sizeof(double)));

      Nlamu=2*2*atmosLocal->Nrays*(spectrumLocal->nJlam+2)*atmosLocal->Nspace;
      if (spectrumLocal->nc != NULL) free((void*)(spectrumLocal->nc-1));
      spectrumLocal->nc=  ((unsigned int*)calloc(Nlamu, sizeof(unsigned int)))+1;
      
      lc = 0, onc = 0, omin = 0;
      lambda = spectrumLocal->Jlam;
      
      for (la = 0;  la < spectrumLocal->nJlam;  la++) {
	for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
	  firstl = true;
	  ...
	  for (to_obs = 0;  to_obs <= 1;  to_obs++) {
	    ...
	    for (k = 0;  k < atmosLocal->Nspace;  k++) {
	      lastl = true;
	      lamuk = la * (atmosLocal->Nrays*2*atmosLocal->Nspace) 
		+ mu     * (2*atmosLocal->Nspace)
		+ to_obs * (atmosLocal->Nspace)
		+ k ;
	      
	      ncoef=0;
	      lc = spectrumLocal->nc[lamuk-1];
	      ...
	      lastl = true;
	      for (idx = 0; idx < spectrumLocal->nJlam ; idx++) {
		
		if ((lambda[idx] > lambda_prv) && (lambda[idx] <= lambda_gas)){
		  
		  ncoef += 1;
		  spectrumLocal->iprdh[lc]=(unsigned int)idx;
		  spectrumLocal->cprdh[lc++]=(float)((lambda[idx]-lambda_prv)/dl);
		  /* if(firstl){ */
		  /*   omin = min(idx, omin); */
		  /*   firstl = false; */
		  /* } */
		...
		if ((lambda[idx] > lambda_gas) && (lambda[idx] < lambda_nxt)){
		  ncoef += 1;
		  spectrumLocal->iprdh[lc]=(unsigned int)idx;
		  spectrumLocal->cprdh[lc++]=(float)(1. - (lambda[idx]-lambda_gas)/dl1);
		...
		if(!lastl) break;
	      ...
	      spectrumLocal->nc[lamuk]=onc+ncoef;
	      onc = spectrumLocal->nc[lamuk];
	      //      }
	...
	//	omin = max(omin-1, 0);
      ...
      if(spectrumLocal->nc[lamuk] > Nlamu){
	fprintf(stderr,"rh::initial_j: ERROR, need to allocate more space for interpolation coeffs. Allocated [%d], needed [%d]\n", Nlamu, spectrumLocal->nc[lamuk]);
	exit(0);
      }
    }
  ...
  if (atmosLocal->Stokes || inputLocal->backgr_pol) {
    if (spectrumLocal->Stokes_Q != NULL) {
      freeMatrix((void **) spectrumLocal->Stokes_Q);
      spectrumLocal->Stokes_Q = NULL;
    }
    if (spectrumLocal->Stokes_U != NULL) {
      freeMatrix((void **) spectrumLocal->Stokes_U);
      spectrumLocal->Stokes_U = NULL;
    }
    if (spectrumLocal->Stokes_V != NULL) {
      freeMatrix((void **) spectrumLocal->Stokes_V);
      spectrumLocal->Stokes_V = NULL;
    }
  }
  
  switch (topology) {
  case ONE_D_PLANE:
    if (spectrumLocal->I != NULL) freeMatrix((void **) spectrumLocal->I);
    spectrumLocal->I = matrix_double(spectrumLocal->Nspect, atmosLocal->Nrays);
    if (atmosLocal->Stokes || inputLocal->backgr_pol) {
      spectrumLocal->Stokes_Q = matrix_double(spectrumLocal->Nspect, atmosLocal->Nrays);
      spectrumLocal->Stokes_U = matrix_double(spectrumLocal->Nspect, atmosLocal->Nrays);
      spectrumLocal->Stokes_V = matrix_double(spectrumLocal->Nspect, atmosLocal->Nrays);
    }
    break;
  case TWO_D_PLANE:
    Nsr = spectrumLocal->Nspect * atmosLocal->Nrays;
    spectrumLocal->I = matrix_double(Nsr, atmosLocal->N[0]);
    if (atmosLocal->Stokes || inputLocal->backgr_pol) {
      spectrumLocal->Stokes_Q = matrix_double(Nsr, atmosLocal->N[0]);
      spectrumLocal->Stokes_U = matrix_double(Nsr, atmosLocal->N[0]);
      spectrumLocal->Stokes_V = matrix_double(Nsr, atmosLocal->N[0]);
    }
    break;
  case THREE_D_PLANE:
    spectrumLocal->I = matrix_double(spectrumLocal->Nspect * atmosLocal->Nrays, 
			       atmosLocal->N[0] * atmosLocal->N[1]);
    if (atmosLocal->Stokes || inputLocal->backgr_pol) {
      Nsr    = spectrumLocal->Nspect * atmosLocal->Nrays;
      Nplane = atmosLocal->N[0] * atmosLocal->N[1];

      spectrumLocal->I = matrix_double(Nsr, Nplane);
      if (atmosLocal->Stokes || inputLocal->backgr_pol) {
	spectrumLocal->Stokes_Q = matrix_double(Nsr, Nplane);
	spectrumLocal->Stokes_U = matrix_double(Nsr, Nplane);
	spectrumLocal->Stokes_V = matrix_double(Nsr, Nplane);
      }
    }
    break;
  case SPHERICAL_SYMMETRIC:
  ...
  if (firsttime) {
    ...
    if (atmosLocal->NPRDactive > 0 && inputLocal->PRD_angle_dep == PRD_ANGLE_DEP) {
      oflag = 0;
      if (inputLocal->startJ == OLD_J) {
	...
	oflag |= (O_RDWR | O_CREAT);
      ...
      if ((spectrumLocal->fd_Imu = open(file_imu, oflag, PERMISSIONS)) == -1) {
	sprintf(messageStr, "Unable to open %s file %s with permission %s",
		(spectrumLocal->updateJ) ? "update" : "input",
		file_imu, permission);
	Error(ERROR_LEVEL_2, routineName, messageStr);
      }
      /* --- Fill the index list that keeps track of the location
	 of intensity Imu in file spectrumLocal->fd_Imu at wavelength
	 corresponding to nspect. --                 -------------- */
      
      spectrumLocal->PRDindex = (int *) malloc(spectrumLocal->Nspect * sizeof(int));
      index = 0;
      for (nspect = 0;  nspect < spectrumLocal->Nspect;  nspect++) {
	if (containsPRDline_ctx(&spectrumLocal->as[nspect], ctx)) {
	  spectrumLocal->PRDindex[nspect] = index++;
	}
      }
    }
...
// extern CommandLine commandline;
  ...
  const char routineName[] = "initSolution_j_ctx";
  register int k, i, ij, nspect, n, nact;
  int     la, j, status;
  double  gijk, wla, twohnu3_c2, hc_k, twoc, fourPI;
  ActiveSet  *as;
  ...
  initSolution_alloc2_ctx(myrank, ctx);
  ...
  for (nact = 0;  nact < atmosLocal->Nactiveatom;  nact++) {
    ...
    case ZERO_RADIATION:
      zeroRadiation_ctx(atom, nact, ctx);
      break;
	...
	if(conv) break;
      ...
      if(!conv) zeroRadiation_ctx(atom, nact, ctx);
      break;
  ...
  for (nact = 0;  nact < atmosLocal->Nactivemol;  nact++) {
    ...
    if (strstr(molecule->ID, "CO"))
      COcollisions_ctx(molecule, ctx);
    else if (strstr(molecule->ID, "H2"))
      H2collisions_ctx(molecule, ctx);
    else {
      sprintf(messageStr, "Collisions for molecule %s not implemented\n",
	      molecule->ID);
      Error(ERROR_LEVEL_2, routineName, messageStr);
