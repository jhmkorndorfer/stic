	     ...
	     int iverbose, int *hydrostat, int computing_derivatives)
{
  ...
  if(!mpi.stop){
	...
	if (solveSpectrum(FALSE, FALSE, 0, TRUE) <= ctx->input.iterLimit) break;
	niter++;
      }
    } else dpopmax = 1.0e13;
  } else dpopmax = 1.0e13;
  ...
  if(savpop > 0 && converged)
    save_populations(ctx, save_pop, ne_lte);
...
void clean_saved_populations(RHContext* ctx, crhpop *save_pop){
  ...
  if(save_pop->nactive > 0){
    ...
    free(save_pop->J);
    if(ctx->input.backgr_pol) free(save_pop->J20);
...
void save_populations(RHContext* ctx, crhpop *save_pop, double *ne_lte){
  ...
  if(save_pop->nactive > 0) clean_saved_populations(ctx, save_pop);

  save_pop->nactive = ctx->atmos.Nactiveatom;
  save_pop->ndep = ctx->atmos.Nspace;
  save_pop->nw = ctx->spectrum.Nspect;
  //
  save_pop->pop =     (crhatom*) malloc(ctx->atmos.Nactiveatom * sizeof(crhatom));
  save_pop->lambda =  (double*) malloc(ctx->spectrum.Nspect*sizeof(double));
  //
  save_pop->J =   (double*) malloc(ctx->spectrum.Nspect* ctx->atmos.Nspace*sizeof(double));
  save_pop->tau_ref = (double*) malloc(ctx->atmos.Nspace*sizeof(double));
  save_pop->ne_dep = (double*) calloc(ctx->atmos.Nspace, sizeof(double));

  
  if(ctx->input.backgr_pol)
    save_pop->J20 = (double*) malloc(ctx->spectrum.Nspect* ctx->atmos.Nspace*sizeof(double));
  else
    save_pop->J20 = NULL;

  memcpy(save_pop->tau_ref, ctx->geometry.tau_ref, sizeof(double)*ctx->atmos.Nspace);
  for(k=0;k<ctx->atmos.Nspace; ++k) save_pop->ne_dep[k] = ctx->atmos.ne[k]/ne_lte[k];
  
  
  /* --- copy J, J20 and lambda ---*/
  
  memcpy(&save_pop->J[0], &ctx->spectrum.J[0][0],
	 ctx->spectrum.Nspect*ctx->atmos.Nspace*sizeof(double));
  
  if(ctx->input.backgr_pol){
    memcpy(&save_pop->J20[0], &ctx->spectrum.J20[0][0],
	   ctx->spectrum.Nspect*ctx->atmos.Nspace*sizeof(double));
  }// else  fprintf(stderr, "NOT STORING J20!\n");
    
  memcpy(&save_pop->lambda[0], ctx->spectrum.lambda, ctx->spectrum.Nspect * sizeof(double));

  /* --- Copy populations for each active atom --- */
  
  for (nact = 0;  nact < ctx->atmos.Nactiveatom;  nact++) {
    
    atom = ctx->atmos.activeatoms[nact];
    save_pop->pop[nact].nlevel = atom->Nlevel;
    save_pop->pop[nact].converged  = true;

    /* --- Allocate arrays ---*/
    save_pop->pop[nact].n = (double*) malloc( atom->Nlevel * ctx->atmos.Nspace*sizeof(double));
    save_pop->pop[nact].ntotal = (double*) malloc(ctx->atmos.Nspace*sizeof(double));


    /* --- copy populations and ntotal---*/
    // memcpy(&save_pop->pop[nact].n[0], &atom->n[0][0],
    //atom->Nlevel*ctx->atmos.Nspace*sizeof(double));
    memcpy(&save_pop->pop[nact].ntotal[0], &atom->ntotal[0], ctx->atmos.Nspace*sizeof(double));

    for(j=0;j<atom->Nlevel;j++)
      for(k=0;k<ctx->atmos.Nspace;k++)
	save_pop->pop[nact].n[j*ctx->atmos.Nspace+k] = atom->n[j][k] / atom->nstar[j][k]; //atom->ntotal[k];
	...
	if (line->PRD){
	  ...
	  save_pop->pop[nact].line[ii].rho = (double*)
	    malloc(line->Nlambda * ctx->atmos.Nspace * sizeof(double));
	  //
	  ...
	  memcpy(save_pop->pop[nact].line[ii].rho, &line->rho_prd[0][0],
		 line->Nlambda*ctx->atmos.Nspace*sizeof(double));
	  ii++;
	} // PRD line
      } // kr
    }// nprd > 0
...
void read_populations(RHContext* ctx, crhpop *save_pop, int flag){
  ...
  register int k, j, la;
  double ratio, tmp;
  double *tmp1 = (double*)malloc(ctx->atmos.Nspace*sizeof(double));
  
  if(save_pop->pop == NULL || save_pop->nactive != ctx->atmos.Nactiveatom){
    // fprintf(stderr,"read_population: ctx->atmos->Nactiveatom[%d] != save_pop->nactive[%d]\n",
    //ctx->atmos.Nactiveatom, save_pop->nactive);
    return;
  }
  ...
  for(nact=0;nact < save_pop->nactive;nact++){
    ...
    /* --- Check dimensions --- */
    if(atom->Nlevel != save_pop->pop[nact].nlevel || ctx->atmos.Nspace != save_pop->ndep){
      continue;
    }
    copied += 1;
    ...
    for(j= 0; j < atom->Nlevel; j++ ){
      hermitian_interpolation((int)ctx->atmos.Nspace, save_pop->tau_ref, &save_pop->pop[nact].n[j*ctx->atmos.Nspace],
      		      (int)ctx->atmos.Nspace, ctx->geometry.tau_ref, tmp1, 1);      
	...
	for(kr = 0; kr<save_pop->pop[nact].nprd; kr++){
	  ...
	  if(line->PRD && (save_pop->pop[nact].line[kr].nlambda == line->Nlambda)){
	    ...
	    for(la=0;la<line->Nlambda;la++){
	      hermitian_interpolation((int)ctx->atmos.Nspace, save_pop->tau_ref, &save_pop->pop[nact].line[kr].rho[la*ctx->atmos.Nspace],
	      			      (int)ctx->atmos.Nspace, ctx->geometry.tau_ref, line->rho_prd[la],0);
	    }
	  }else{
	...
	fprintf(ctx->commandline.logfile,"read_populations: nprd[%d] != save_pop.pop.nprd[%d], not copying PRD rho!\n", nprd,save_pop->pop[nact].nprd );
      }
  ...
  if(ctx->spectrum.Nspect == save_pop->nw || ctx->atmos.Nspace == save_pop->ndep){
    for(la=0;la<ctx->spectrum.Nspect;la++){
      hermitian_interpolation((int)ctx->atmos.Nspace, save_pop->tau_ref, &save_pop->J[la*ctx->atmos.Nspace],
      			      (int)ctx->atmos.Nspace, ctx->geometry.tau_ref, ctx->spectrum.J[la],0);
      //memcpy(ctx->spectrum.J[la], &save_pop->J[la*ctx->atmos.Nspace], ctx->atmos.Nspace*sizeof(double));
    
    
      if(ctx->input.backgr_pol){
	hermitian_interpolation((int)ctx->atmos.Nspace, save_pop->tau_ref, &save_pop->J20[la*ctx->atmos.Nspace],
				      (int)ctx->atmos.Nspace, ctx->geometry.tau_ref, ctx->spectrum.J20[la],0);
	//memcpy(ctx->spectrum.J20[la], &save_pop->J20[la*ctx->atmos.Nspace], ctx->atmos.Nspace*sizeof(double));
	
	//memcpy(&ctx->spectrum.J20[0][0], &save_pop->J20[0],
	//	     ctx->spectrum.Nspect*ctx->atmos.Nspace*sizeof(double));
