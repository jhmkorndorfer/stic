...
double Feautrier_ctx(int nspect, int mu, double *chi, double *S, enum FeautrierOrder F_order, double *P, double *Psi, RHContext *ctx)
{
  ...
  for (k = 0;  k < Ndep-1;  k++) {
    dtau[k] = zmu * (chi[k] + chi[k+1]) *
      (geometryLocal->height[k] - geometryLocal->height[k+1]);
  }
  ...
  case IRRADIATED:
    r0 = 0.0;
    h0 = geometryLocal->Itop[nspect][mu];
    break;
  case REFLECTIVE:
    r0 = 1.0;
    h0 = 0.0;
  ...
  case IRRADIATED:
    rN = 0.0;
    hN = geometryLocal->Ibottom[nspect][mu];
    break;
  case REFLECTIVE:
    rN = 1.0;
    hN = 0.0;
