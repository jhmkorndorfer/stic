...
{
  const char routineName[] = "PRDScatter_ctx";
  register int  la, k, lap, kr, ip, kxrd;
  ...
  Pj    = (double *) malloc (atmosLocal->Nspace * sizeof(double));
  adamp = (double *) malloc (atmosLocal->Nspace * sizeof(double)); 
  cDop  = (NM_TO_M * PRDline->lambda0) / (4.0 * PI);
  ...
  for (kxrd = 0;  kxrd < Nsubordinate;  kxrd++) {
	...
	XRDline->Bij / Pj[k];
      ...
      for (la = 0;  la < XRDline->Nlambda;  la++) {
	J_k[la]   = spectrumLocal->J[XRDline->Nblue + la][k];
	q_abs[la] = (XRDline->lambda[la] - XRDline->lambda0) * CLIGHT /
	  (XRDline->lambda0 * atom->vbroad[k]);
      }
      switch (representation) {
...
void PRDAngleApproxScatter_ctx(AtomicLine *PRDline, enum Interpolation representation, RHContext *ctx)
{
  ...
  for (kxrd = 0;  kxrd < Nsubordinate;  kxrd++) {
	...
	XRDline->Bij / Pj[k];
      ...
      for (la = 0;  la < XRDline->Nlambda;  la++) {
	//J_k[la]   = spectrumLocal->Jgas[XRDline->Nblue + la][k];
	J_k[la]   = XRDline->Jgas[la][k];//spectrumLocal->linfo[nspect]->line->Jgas[la][k];
	...
	case LINEAR:
	  ...
	  break;
	case SPLINE:
	  splineEval(Np, (XRDline->symmetric) ? qpp : qp, J, hunt=TRUE); // POSSIBLE BUGS HERE!! Race condition

	  break;
	case EXP_SPLINE:
	  exp_splineEval(Np, (XRDline->symmetric) ? qpp : qp, J, hunt=TRUE); // POSSIBLE BUGS HERE!! Race condition
	  break;
	}
	/* --- Compute the redistribution weights --   -------------- */
...
/* ------- end ---------------------------- PRDAngleApproxScatter_ctx.c - */
...
void PRDAngleScatter_ctx(AtomicLine *PRDline, enum Interpolation representation, RHContext *ctx)
{
  ...
  Imup    = matrix_double(PRDline->Nlambda, atmosLocal->Nspace);
  Ik      = (double *) malloc(PRDline->Nlambda * sizeof(double));
  v_emit  = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  v_abs   = (double *) malloc(PRDline->Nlambda * sizeof(double));
  adamp   = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  v_los   = matrix_double(atmosLocal->Nrays, atmosLocal->Nspace);
  gamma   = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  Pj      = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  Jbar    = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  RIInorm = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  ...
  for (kr = 0;  kr < atom->Nline;  kr++) {
    line = &atom->line[kr];
    if (line->j == PRDline->j)
      for (k = 0;  k < atmosLocal->Nspace;  k++) Pj[k] += line->Rji[k];
    if (line->i == PRDline->j)
      for (k = 0;  k < atmosLocal->Nspace;  k++) Pj[k] += line->Rij[k];
  }
  for (kr = 0;  kr < atom->Ncont;  kr++) {
    continuum = &atom->continuum[kr];
    if (continuum->j == PRDline->j)
      for (k = 0;  k < atmosLocal->Nspace;  k++) Pj[k] += continuum->Rji[k];
    if (continuum->i == PRDline->j)
      for (k = 0;  k < atmosLocal->Nspace;  k++) Pj[k] += continuum->Rij[k];
  }
  for (k = 0;  k < atmosLocal->Nspace;  k++) {
    gamma[k] = atom->n[PRDline->i][k] / atom->n[PRDline->j][k] *
      PRDline->Bij / Pj[k];
  ...
  for (mu = 0;  mu < atmosLocal->Nrays;  mu++) {
    for (k = 0;  k < atmosLocal->Nspace;  k++)
      v_los[mu][k] = vproject_ctx(k, mu, ctx) / atom->vbroad[k];
  }
  /* --- Depth-dependent damping parameter --          -------------- */
  ...
  for (la = 0;  la < PRDline->Nlambda;  la++) {
	...
	if (atmosLocal->moving ||
	    (PRDline->polarizable && inputLocal->StokesMode > FIELD_FREE))
	  phi_emit = PRDline->phi[lamu];
	else
	  phi_emit = PRDline->phi[la];
	...
	for (mup = 0;  mup < atmosLocal->Nrays;  mup++) {
	  wmup = 0.5 * atmosLocal->wmu[mup];
	  for (to_obs_p = 0;  to_obs_p <= 1;  to_obs_p++) {
	    ...
	    /* --- Read specific intensity in this direction for all
            ...
            for (lap = 0;  lap < PRDline->Nlambda;  lap++)
	      readImu_ctx(PRDline->Nblue + lap, mup, to_obs_p, Imup[lap], ctx);
		...
		if (to_obs_p)
	      ...
	      switch (representation) {
	      case SPLINE:
		splineCoef(PRDline->Nlambda, v_abs, Ik); // POSSIBLE BUGS HERE race condition
		break;
	      case EXP_SPLINE:
		exp_splineCoef(PRDline->Nlambda, v_abs, Ik, TENSION); // POSSIBLE BUGS HERE race condition
		break;
	      case LINEAR: break;
	      }
	      /* --- Establish integration limits over absorption
		...
		break;
	      case SPLINE:
		splineEval(Np, vp, I, hunt=TRUE); // POSSIBLE BUGS HERE race condition
		break;
	      case EXP_SPLINE:
		exp_splineEval(Np, vp, I, hunt=TRUE); // POSSIBLE BUGS HERE race condition
		break;
	      }
	      /* --- Compute the redistribution weights -- ---------- */
