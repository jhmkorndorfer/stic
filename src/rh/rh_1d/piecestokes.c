...
void PiecewiseStokes_ctx(int nspect, int mu, bool_t to_obs, double *chi_I, double **S, double **I, double *Psi, RHContext *ctx)
{
  ...
  if(FALSE){
    chi_I1 = (double*)malloc(Ndep * sizeof(double));//scl_opac(Ndep, chi);
    for(i=0;i<Ndep;i++) chi_I1[i] = chi_I[i] / atmosLocal->rho[i];
  ...
  StokesK_ctx(nspect, k_start, chi_I[k_start], K_upw, ctx);
  ...
  for (k = k_start+dk;  k != k_end+dk;  k += dk) {
    w3(dtau_uw, w);
    StokesK_ctx(nspect, k, chi_I[k], K, ctx);
