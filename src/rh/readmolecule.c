...
void readMolecule_ctx(Molecule *molecule, char *fileName, bool_t active, RHContext *ctx)
{
  ...
  for (n = 0;  n < molecule->Nelement;  n++) {
    ...
    match = FALSE;
    for (m = 0;  m < atmosLocal->Nelem;  m++) {
      if (strstr(elementID, atmosLocal->elements[m].ID)) {
	if (atmosLocal->elements[m].abundance_set) {
	  molecule->pt_index[n] = m;
	  match = TRUE;
	}
	break;
  ...
  molecule->pf = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  ...
  for (n = 0;  n < molecule->Nelement;  n++)
    molecule->weight += molecule->pt_count[n] *
      atmosLocal->elements[molecule->pt_index[n]].weight;

  /* --- Allocate memory for total population numbers, broadening
         velocity and partition function --            -------------- */

  molecule->n = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  molecule->vbroad = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  ...
  while (getLine(fp_molecule, COMMENT_CHAR, inputLine,
		 exit_on_EOF=FALSE) != EOF) {
    Nread = sscanf(inputLine, "%s", line_data);
    readMolecularLines_ctx(molecule, line_data, ctx);
  }
  /* --- Sort transitions within each molecule in wavelength -- ----- */
  ...
  if (molecule->active) {
    for (kr = 0;  kr < molecule->Nrt;  kr++) {
      mrt = &molecule->mrt[kr];
      dlambda_wing = mrt->qwing * atmosLocal->vmicro_char / CLIGHT;
      dlambda = 2.0*dlambda_wing * mrt->lambda0 / (mrt->Nlambda - 1);
    ...
    molecule->rhth =
      (rhthread *) malloc(inputLocal->Nthreads * sizeof(rhthread));
  }
...
void readMolecularLines_ctx(struct Molecule *molecule, char *line_data, RHContext *ctx)
{
  ...
  if (strstr(format_string, "GOORVITCH94")) {
	...
	if (atmosLocal->Stokes) mrt->polarizable = TRUE;
      } else
	mrt->polarizable = FALSE;
    }
  } else {
...
/* ------- begin -------------------------- readMolecularModels_ctx.c --- */

void readMolecularModels_ctx(RHContext *ctx)
{
  const char routineName[] = "readMolecularModels_ctx";
  register int n, m, i;
  ...
  if ((fp_molecules = fopen(inputLocal->molecules_input, "r")) == NULL) {
    sprintf(messageStr, "Unable to open input file %s",
	    inputLocal->molecules_input);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  }
  /* --- Get the number of molecular models to be read -- ----------- */

  getLine(fp_molecules, COMMENT_CHAR, inputLine, exit_on_EOF=TRUE);
  Nread = sscanf(inputLine, "%d", &atmosLocal->Nmolecule);
  checkNread(Nread, Nrequired=1, routineName, checkPoint=1);

  atmosLocal->molecules =
    (Molecule *) malloc(atmosLocal->Nmolecule * sizeof(Molecule));
  ...
  for (n = 0;  n < atmosLocal->Nmolecule;  n++) {
    ...
    for (m = 0;  m < n;  m++) {
      if (strcmp(moleculeID, atmosLocal->molecules[m].ID) == 0) {
	sprintf(messageStr, "Duplicate model for molecule %s\n",
		moleculeID);
	Error(ERROR_LEVEL_2, routineName, messageStr);
      }
    ...
    printf("[%d] Calling readMolecule_ctx on file: %s\n", n, filename);
    molecule = &atmosLocal->molecules[n];

    readMolecule_ctx(molecule, filename, active=(strstr(actionKey, "ACTIVE") ? TRUE : FALSE), ctx);

    printf("Read successfully molecule %s\n", filename);

    /* --- Create an alias to the hydrogen molecule  structure -- ----- */

    if (strcmp(molecule->ID, "H2") == 0) atmosLocal->H2 = molecule;

    /* --- Store pointers to OH and CH molecules, if appropriate,
           for use in bf opacity calculation (see ohchbf.c) -- ------ */                      

    if (strcmp(molecule->ID, "OH") == 0) atmosLocal->OH = molecule;
    if (strcmp(molecule->ID, "CH") == 0) atmosLocal->CH = molecule;
  ...
  for (n = 0;  n < atmosLocal->Nelem;  n++) {
    element = &atmosLocal->elements[n];
    element->Nmolecule = 0;
    element->mol_index = (int *) malloc(atmosLocal->Nmolecule * sizeof(int));

    for (m = 0;  m < atmosLocal->Nmolecule;  m++) {
      molecule = &atmosLocal->molecules[m];
      for (i = 0;  i < molecule->Nelement;  i++) {
	if (molecule->pt_index[i] == n) {
	  element->mol_index[element->Nmolecule++] = m;
	  break;
  ...
  atmosLocal->nHmin = (double *) malloc(atmosLocal->Nspace * sizeof(double));

  getCPU(2, TIME_POLL, "Read molecular input");
}
/* ------- end ---------------------------- readMolecularModels_ctx.c --- */
