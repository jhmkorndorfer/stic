...
void FixedRate_ctx(Atom *atom, RHContext *ctx)
{
  ...
  e_mc = (Q_ELECTRON / EPSILON_0) * (Q_ELECTRON / (CLIGHT*M_ELECTRON));
  if (topology == TWO_D_PLANE) {
    Nrepeat = atmosLocal->N[0];
    Ndepth  = atmosLocal->N[1];
  } else if (topology == THREE_D_PLANE) {
    Nrepeat = atmosLocal->N[0] * atmosLocal->N[1];
    Ndepth  = atmosLocal->N[2];
  } else {
    Nrepeat = 1;
    Ndepth  = atmosLocal->Nspace;
  }
  /* --- Go through the fixed transitions --           -------------- */
  ...
  for (kf = 0;  kf < atom->Nfixed;  kf++) {
	...
	deltaT = Te - atmosLocal->T[k + Nrepeat];
