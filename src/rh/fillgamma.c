...
void addtoGamma_ctx(int nspect, double wmu, double *I, double *Psi, RHContext *ctx)
{
  ...
  if (containsActive_ctx(as, ctx)) {
    Ieff = (double *) malloc(atmosLocal->Nspace * sizeof(double));
    ...
    if (inputLocal->StokesMode == FULL_STOKES  &&  containsPolarized_ctx(as, ctx)) {
      ...
      Stokes_Q = I + atmosLocal->Nspace;
      Stokes_U = I + 2*atmosLocal->Nspace;
      Stokes_V = I + 3*atmosLocal->Nspace;
    }
  }
  /* --- Contributions from the active transitions in atoms -- ------ */
  ...
  for (nact = 0;  nact < atmosLocal->Nactiveatom;  nact++) {
	...
	i = continuum->i;
	j = continuum->j;
	twohnu3_c2 = twohc / CUBE(spectrumLocal->lambda[nspect]);
	break;
	...
	twohnu3_c2 = 0.0;
      ...
      if (inputLocal->Nthreads > 1) pthread_mutex_lock(&atom->Gamma_lock);
	...
	if (jp == i) {
      ...
      if (inputLocal->Nthreads > 1) pthread_mutex_unlock(&atom->Gamma_lock); 
    }
  }
  /* --- Add the active molecular contributions --     -------------- */
  ...
  for (nact = 0;  nact < atmosLocal->Nactivemol;  nact++) {
	...
	twohnu3_c2 = 0.0;
      ...
      if (inputLocal->Nthreads > 1) pthread_mutex_lock(&molecule->Gamma_lock);
	...
	if (molecule->n[k]) {
      ...
      if (inputLocal->Nthreads > 1) pthread_mutex_unlock(&molecule->Gamma_lock);
    }
  }

  if (containsActive_ctx(as, ctx)) free(Ieff);
}
/* ------- end ---------------------------- addtoGamma_ctx.c ------------ */
...
/* ------- begin -------------------------- addtoCoupling_ctx.c --------- */

void addtoCoupling_ctx(int nspect, RHContext *ctx)
{
  const  char routineName[] = "addtoCoupling_ctx";
  register int nact, n, k;
  Atmosphere *atmosLocal = &ctx->atmos;
  Spectrum *spectrumLocal = &ctx->spectrum;
  InputData *inputLocal = &ctx->input;
...
void addtoRates_ctx(int nspect, int mu, bool_t to_obs, double wmu, double *I, bool_t redistribute, RHContext *ctx)
{
  ...
  for (nact = 0;  nact < atmosLocal->Nactiveatom;  nact++) {
	...
	Rij = NULL;
      ...
      if (Rij != NULL) {
	if (inputLocal->Nthreads > 1) pthread_mutex_lock(rate_lock);
	...
	if (inputLocal->Nthreads > 1) pthread_mutex_unlock(rate_lock);
      }
    }
  }
}
/* ------- end ---------------------------- addtoRates_ctx.c ------------ */
