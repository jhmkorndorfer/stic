...
{
  const char routineName[] = "readInput_ctx";
  static char atom_input[MAX_VALUE_LENGTH], molecule_input[MAX_VALUE_LENGTH];
  ...
  Keyword theKeywords[] = {
    {"ATMOS_FILE", "", FALSE, KEYWORD_REQUIRED, ctx->input.atmos_input,
     setcharValue},
    {"ABUND_FILE", "", FALSE, KEYWORD_REQUIRED, ctx->input.abund_input,
     setcharValue},

    {"NRAYS",     "0", FALSE, KEYWORD_OPTIONAL, &ctx->atmos.Nrays, setintValue},
    {"ANGLE_SET", "NO_SET", FALSE, KEYWORD_OPTIONAL, &ctx->atmos.angleSet,
     setAngleSet},

    {"EDDINGTON", "FALSE", FALSE, KEYWORD_OPTIONAL, &ctx->input.Eddington,
     setboolValue},
    {"ATMOS_ITOP", "none", FALSE, KEYWORD_OPTIONAL, ctx->input.Itop, setcharValue},

    {"WAVETABLE", "none", FALSE, KEYWORD_OPTIONAL, ctx->input.wavetable_input,
     setcharValue},
    {"ATOMS_FILE",  "", FALSE, KEYWORD_REQUIRED, ctx->input.atoms_input,
     setcharValue},
    {"MOLECULES_FILE",  "", FALSE, KEYWORD_REQUIRED, ctx->input.molecules_input,
     setcharValue},
    {"NON_ICE", "FALSE", FALSE, KEYWORD_OPTIONAL, &ctx->input.NonICE,
     setboolValue},

    {"N_MAX_SCATTER", "5", FALSE, KEYWORD_OPTIONAL, &ctx->input.NmaxScatter,
     setintValue},

    {"I_SUM",     "0", FALSE, KEYWORD_REQUIRED, &ctx->input.isum, setintValue},
    {"N_MAX_ITER", "", FALSE, KEYWORD_REQUIRED, &ctx->input.NmaxIter,
     setintValue},
    {"ITER_LIMIT", "", FALSE, KEYWORD_REQUIRED, &ctx->input.iterLimit,
     setdoubleValue},
    {"NG_DELAY",  "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.Ngdelay, setintValue},
    {"NG_ORDER",  "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.Ngorder, setintValue},
    {"NG_PERIOD", "1", FALSE, KEYWORD_OPTIONAL, &ctx->input.Ngperiod,
     setintValue},
    {"NG_MOLECULES", "FALSE", FALSE, KEYWORD_DEFAULT, &ctx->input.accelerate_mols,
     setboolValue},
    {"PRD_N_MAX_ITER", "3", FALSE, KEYWORD_OPTIONAL, &ctx->input.PRD_NmaxIter,
     setintValue},
    {"PRD_ITER_LIMIT", "1.0E-2", FALSE, KEYWORD_OPTIONAL, &ctx->input.PRDiterLimit,
     setdoubleValue},
    {"NG_START_LIMIT", "1.0E-2", FALSE, KEYWORD_OPTIONAL, &ctx->input.ng_start_limit,
     setdoubleValue},
    {"PRD_NG_DELAY",  "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.PRD_Ngdelay,
     setintValue},
    {"PRD_NG_ORDER",  "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.PRD_Ngorder,
     setintValue},
    {"PRD_NG_PERIOD", "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.PRD_Ngperiod,
     setintValue},
    {"PRD_ANGLE_DEP", "0", FALSE, KEYWORD_DEFAULT, &ctx->input.PRD_angle_dep,
     setPRDangle},
    {"XRD", "FALSE", FALSE, KEYWORD_DEFAULT, &ctx->input.XRD, setboolValue}, 
    {"N_LAMBDA_ITER",  "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.NlambdaIter,
     setintValue},
    {"J_FILE",     "", FALSE, KEYWORD_REQUIRED, &ctx->input.JFile, setcharValue},
    {"BACKGROUND_FILE", "", FALSE, KEYWORD_REQUIRED, &ctx->input.background_File,
     setcharValue},
    {"BACKGROUND_RAY_FILE", "background.ray", FALSE,
     KEYWORD_OPTIONAL, &ctx->input.background_ray_File,
     setcharValue},
    {"OLD_BACKGROUND", "FALSE", FALSE, KEYWORD_OPTIONAL,
     &ctx->input.old_background, setboolValue},
    {"STARTING_J", "", FALSE, KEYWORD_REQUIRED, &ctx->input.startJ,
     setstartValue},
    {"HYDROGEN_LTE", "TRUE", FALSE, KEYWORD_DEFAULT, &ctx->atmos.H_LTE,
     setboolValue},
    {"HYDROSTATIC", "FALSE", FALSE, KEYWORD_DEFAULT, &ctx->atmos.hydrostatic,
     setboolValue},
    {"KURUCZ_DATA", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.KuruczData,
     setcharValue},
    {"RLK_SCATTER", "FALSE", FALSE, KEYWORD_DEFAULT, &ctx->input.rlkscatter,
     setboolValue},
    {"KURUCZ_PF_DATA", "Atoms/pf_Kurucz.input", FALSE,
     KEYWORD_REQUIRED, &ctx->input.pfData, setcharValue},
    {"SOLVE_NE", "NONE", FALSE, KEYWORD_DEFAULT, &ctx->input.solve_ne,
     setnesolution},
    {"OPACITY_FUDGE", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.fudgeData,
     setcharValue},
    {"METALLICITY", "0.0", FALSE, KEYWORD_DEFAULT, &ctx->input.metallicity,
     setdoubleValue},

    {"ATMOS_OUTPUT", "atmos.out", FALSE, KEYWORD_DEFAULT, &ctx->input.atmos_output,
     setcharValue},
    {"GEOMETRY_OUTPUT", "geometry.out", FALSE, KEYWORD_OPTIONAL,
     &ctx->input.geometry_output, setcharValue},
    {"SPECTRUM_OUTPUT", "spectrum.out", FALSE, KEYWORD_DEFAULT,
       &ctx->input.spectrum_output, setcharValue},
    {"OPACITY_OUTPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.opac_output,
     setcharValue},
    {"RADRATE_OUTPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.radrateFile,
     setcharValue},
    {"COLLRATE_OUTPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.collrateFile,
     setcharValue},
    {"DAMPING_OUTPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.dampingFile,
     setcharValue},
    {"COOLING_OUTPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.coolingFile,
     setcharValue},

    {"VMICRO_CHAR", "",     FALSE, KEYWORD_REQUIRED, &ctx->atmos.vmicro_char,
     setdoubleValue},
    {"VMACRO_TRESH", "0.0", FALSE, KEYWORD_OPTIONAL, &ctx->atmos.vmacro_tresh,
     setdoubleValue},
    {"LAMBDA_REF",   "500.0", FALSE, KEYWORD_DEFAULT, &ctx->atmos.lambda_ref,
     setdoubleValue},
    {"VACUUM_TO_AIR", "0", FALSE, KEYWORD_OPTIONAL, &ctx->spectrum.vacuum_to_air,
     setboolValue},

    /* --- Magnetic field related inputs go here --     ------------- */

    {"STOKES_INPUT", "none", FALSE, KEYWORD_OPTIONAL, &ctx->input.Stokes_input,
     setcharValue},
    {"B_STRENGTH_CHAR", "0.0", FALSE, KEYWORD_DEFAULT, &ctx->atmos.B_char,
     setdoubleValue},
    {"STOKES_MODE", "NO_STOKES", FALSE, KEYWORD_OPTIONAL,
     &ctx->input.StokesMode, setStokesMode},
    {"MAGNETO_OPTICAL", "TRUE", FALSE, KEYWORD_DEFAULT,
     &ctx->input.magneto_optical, setboolValue},
    {"FAST_ISOTOPIC_SPLITTING", "TRUE", FALSE, KEYWORD_DEFAULT,
     &ctx->input.fast_isotopic_split, setboolValue},
    {"BACKGROUND_POLARIZATION", "FALSE", FALSE, KEYWORD_DEFAULT,
     &ctx->input.backgr_pol, setboolValue},
    {"RLK_EXPLICIT", "FALSE", FALSE, KEYWORD_DEFAULT,
     &ctx->input.RLK_explicit, setboolValue},
    {"XDR_ENDIAN", "TRUE", FALSE, KEYWORD_OPTIONAL,
     &ctx->input.xdr_endian, setboolValue},

    {"S_INTERPOLATION", "BEZIER3", FALSE, KEYWORD_DEFAULT,
     &ctx->input.S_interpolation, set_S_Interpolation},
    {"S_INTERPOLATION_STOKES", "DELO_BEZIER3", FALSE, KEYWORD_DEFAULT,
     &ctx->input.S_interpolation_stokes, set_S_Interpolation_stokes},

    {"INTERPOLATE_3D", "BICUBIC_3D", FALSE, KEYWORD_DEFAULT,
     &ctx->input.interpolate_3D, setInterpolate_3D},

    {"PRINT_CPU", "0", FALSE, KEYWORD_OPTIONAL, &ctx->stats.printCPU,
     setboolValue},
    {"N_THREADS", "0", FALSE, KEYWORD_OPTIONAL, &ctx->input.Nthreads,
     setThreadValue},
    {"COLLRAD_SWITCH",     "0.0", FALSE, KEYWORD_OPTIONAL, &ctx->input.crsw,
     setdoubleValue},
    {"COLLRAD_SWITCH_INI", "1.0", FALSE, KEYWORD_OPTIONAL, &ctx->input.crsw_ini,
     setdoubleValue},
    {"PRD_SWITCH",     "0.0", FALSE, KEYWORD_OPTIONAL, &ctx->input.prdsw,
     setdoubleValue},
    {"EOS_ITER_LIMIT",  "1.E-2", FALSE, KEYWORD_OPTIONAL, &ctx->input.eos_iter_limit,
     setdoubleValue},
    {"LIMIT_MEMORY", "FALSE", FALSE, KEYWORD_DEFAULT, &ctx->input.limit_memory,
     setboolValue},
    {"ALLOW_PASSIVE_BB", "TRUE", FALSE, KEYWORD_DEFAULT,
     &ctx->input.allow_passive_bb, setboolValue}
  };
  Nkeyword = sizeof(theKeywords) / sizeof(Keyword);

  /* --- Open the input data file --                    ------------- */
  strncpy(ctx->commandline.keyword_input, commandline.keyword_input, MAX_VALUE_LENGTH - 1);
  ctx->commandline.keyword_input[MAX_VALUE_LENGTH - 1] = '\0';

  printf("Reading input from TESTTTTT %s\n", ctx->commandline.keyword_input);

  if ((fp_keyword = fopen(ctx->commandline.keyword_input, "r")) == NULL) {
    sprintf(messageStr, "Unable to open inputfile %s",
	    ctx->commandline.keyword_input);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  }
  ...
  switch (topology) {
  case ONE_D_PLANE:
    if (ctx->atmos.Nrays == 0) {
      Error(ERROR_LEVEL_2, routineName,
	    "Must set keyword NRAYS in 1-D plane parallel geometry");
    }
    if (ctx->atmos.angleSet.set != NO_SET) {
      Error(WARNING, routineName,
	    "Ignoring value of keyword ANGLE_SET in 1-D plane geometry");
    }
    break;
  ...
  case SPHERICAL_SYMMETRIC:
    if (ctx->atmos.Nrays > 0 || ctx->atmos.angleSet.set != NO_SET) {
      Error(WARNING, routineName,
	    "Ignoring value of keywords ANGLE_SET and NRAYS in "
            "spherical geometry");
    }
  ...
  case TWO_D_PLANE:
    if (ctx->input.Eddington && ctx->atmos.angleSet.set != NO_SET) {
      Error(WARNING, routineName,
	    "Ignoring value of keywords ANGLE_SET > NO_VERTICAL when\n "
	    " EDDINGTON is set to TRUE\n"
	    " Using SET_VERTICAL with muz = 1/sqrt(3)");
      ctx->atmos.angleSet.set = SET_VERTICAL;
    }
    break;
  ...
  if (strcmp(ctx->input.Stokes_input, "none")) {
    switch (topology) {
    case ONE_D_PLANE:
    case TWO_D_PLANE:
      if (ctx->atmos.B_char == 0.0) {
	Error(WARNING, routineName,
	      "Parameter atmos.B_char not set or set to zero\n"
	      " Wavelength grids of line profiles do not take account "
	      " of Zeeman splitting");
  ...
  if (ctx->commandline.showkeywords) showValues(Nkeyword, theKeywords);

  /* --- Convert to MKSA units where necessary --       ------------- */

  ctx->atmos.vmicro_char  *= KM_TO_M;
  ctx->atmos.vmacro_tresh *= KM_TO_M;
}
