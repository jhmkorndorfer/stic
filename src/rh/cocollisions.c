...
{
  const char routineName[] = "COcollisions_ctx";
  register int k;
  ...
  hcomega_k = HPLANCK * CLIGHT * OMEGA0 / KBOLTZMANN;
  beta = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  for (k = 0;  k < atmosLocal->Nspace;  k++)
    beta[k] = hcomega_k / atmosLocal->T[k];
  ...
  molecule->C_ul = (double *) malloc(atmosLocal->Nspace * sizeof(double));
  ...
  for (k = 0;  k < atmosLocal->Nspace;  k++) {
    if (molecule->n[k]) {
      ...
      molecule->C_ul[k] = C_0 * atmosLocal->T[k] *
	exp(B_H - A_H * pow(atmosLocal->T[k], -0.33333333)) /
	(1.0 - exp(-beta[k])) * atmosLocal->H->n[0][k];

      /* --- Neutral helium contribution
	     (assuming all helium is neutral) --       -------------- */

      molecule->C_ul[k] += C_0 * atmosLocal->T[k] *
	exp(B_HE - A_HE * pow(atmosLocal->T[k], -0.33333333)) /
	(1.0 - exp(-beta[k])) * atmosLocal->elements[1].abund * atmosLocal->nHtot[k];

      /* --- H2 contribution --                        -------------- */

      molecule->C_ul[k] += C_0 * atmosLocal->T[k] *
	exp(B_H2 - A_H2 * pow(atmosLocal->T[k], -0.33333333)) /
	(1.0 - exp(-beta[k])) * atmosLocal->H2->n[k];

      /* --- Electronic contribution --                -------------- */

      molecule->C_ul[k] += (1.4E-09 * CUBE(CM_TO_M)) / sqrt(beta[k]) *
	((1.0 + beta[k]) + 19.0*exp(-3.22*beta[k]) *
	 (1.0 + 4.22*beta[k])) * atmosLocal->ne[k];
    }
  }
  free(beta);
}
/* ------- end ---------------------------- COcollisions_ctx.c ---------- */
