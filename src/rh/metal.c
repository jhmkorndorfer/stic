...
{  
  ...
  for (m = 0, metal = metals;  m < Nmetal;  m++, metal++) {
	...
	if (lambda <= lambdaEdge  &&  lambda >= continuum->lambda[0]) {
	  ...
	  if (expla == NULL) {
	    expla = (double *) malloc(atmosLocal->Nspace * sizeof(double));
	    for (k = 0;  k < atmosLocal->Nspace;  k++)
	      expla[k] = exp(-hc_kla/atmosLocal->T[k]);
	  }
...
{  
  ...
  for (m = 0;  m < atmosLocal->Natom;  m++) {
    atom = atmosLocal->atoms + m;
    if (!atom->active) {
	...
	j = line->j;
	dlambda = line->lambda0 *
	  line->qwing * (atmosLocal->vmicro_char / CLIGHT);
